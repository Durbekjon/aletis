...
export class AiResponseHandlerService {
  ...
  /**
   * Execute a tool call generated by Gemini and return the result.
   */
    ...
    try {
      ...
      switch (toolName) {
        case 'search_products':
          ...
          const { matches } = await this.productSearchService.search(
            organizationId,
            customer.id,
            searchQuery,
          ...
          if (matches.length === 0) {
            return { status: 'success', products: [], message: 'No products found matching the query.' };
          }
          ...
          return {
             status: 'success',
             products: matches.map(m => ({ id: m.id, caption: m.caption })),
             _sideEffects: { productCards }
          };
          
        case 'create_order':
          // Extract the inner logic of handleCreateOrderIntent
          const order = await this.ordersService.createFromAIResponse(args, customer, organizationId);
          ...
          if (order.orderItems && Array.isArray(order.orderItems)) {
            ...
            itemNames = args.items.map((item: any) => `Product ID ${item.productId} (${item.quantity} qty)`);
          }
          ...
          return { 
            ...
            items: itemNames,
            _sideEffects: { orderCreated: true, orderId: order.id }
          };
          
        case 'get_product':
          const product = await this.productSearchService['prisma'].product.findUnique({
             where: { id: args.productId, organizationId }
          });
          if (!product) return { status: 'error', message: 'Product not found' };
          return { status: 'success', product: { id: product.id, name: product.name, price: product.price, quantity: product.quantity } };
          
        case 'check_inventory':
          const invProduct = await this.productSearchService['prisma'].product.findUnique({
             where: { id: args.productId, organizationId }
          });
          if (!invProduct) return { status: 'error', message: 'Product not found' };
          return { status: 'success', inStock: invProduct.quantity > 0, quantity: invProduct.quantity };
          
        case 'get_customer_orders':
          const orders = await this.ordersService.getOrdersForAI(organizationId, customer.id);
          return { status: 'success', orders };
          
        case 'cancel_order':
          try {
            const cancelledOrder = await this.ordersService.cancelOrder(args.orderId, customer.id, organizationId);
            return { status: 'success', orderId: cancelledOrder.id, message: 'Order cancelled successfully' };
          } catch (e: any) {
        ...
        case 'escalate_to_human':
          const ticket = await this.supportService.createFromAIResponse(
            { reason: args.reason, sentiment: args.sentiment },
            customer,
            organizationId
          );
          return { status: 'success', ticketId: ticket.id, message: 'Escalated to human support.' };
