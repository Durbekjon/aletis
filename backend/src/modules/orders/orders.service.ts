...
export class OrdersService {
  ...
  async updateOrder(
    ...
    if (orderStatus && before && before.status !== orderStatus) {
      ...
      // Schedule follow-up when order reaches a terminal positive status
      if (
        orderStatus === OrderStatus.DELIVERED ||
        orderStatus === OrderStatus.CONFIRMED
      ) {
        this.customerIntelligenceService
          .enqueueFollowUp(orderId)
          .catch((err) =>
            this.logger.warn(`Failed to enqueue follow-up for order ${orderId}: ${err.message}`),
          );
      }
    }
