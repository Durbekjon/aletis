...
export class AnalyticsScheduler {
  ...
  async computeDailyAggregates(): Promise<void> {
    ...
    for (const org of orgs) {
      ...
      // Flush monthly usage snapshot from Redis to DB
      if (this.usageService) {
        await this.usageService.flushMonthlySnapshot(org.id).catch((err) =>
          this.logger.warn(`Usage flush failed for org ${org.id}: ${err.message}`),
        );
      }

      // On the 1st of the month generate invoices for active subscriptions
      if (from.getUTCDate() === 1 && this.billingService) {
        await this.billingService.generateMonthlyInvoice(org.id).catch((err) =>
          this.logger.warn(`Invoice generation failed for org ${org.id}: ${err.message}`),
        );
      }
    }
  }
